package eu.mcuniverse.universeapi.serialization;
...
//			itemJson.addProperty("data", itemStack.getDurability());
		...
		if (itemStack.hasItemMeta()) {
			...
			if (meta instanceof Damageable) {
				Damageable dmageMeta = (Damageable) meta;
				metaJson.addProperty("damage", dmageMeta.getDamage());
			}
...
//					enchants.add(new JsonPrimitive(enchantment.getName() + ":" + integer));
					enchants.add(new JsonPrimitive(
							enchantment.getKey().getNamespace() + ":" + enchantment.getKey().getKey() + ":" + integer));
				});
...
//						storedEnchants.add(new JsonPrimitive(enchantment.getName() + ":" + integer));
						storedEnchants.add(new JsonPrimitive(
								enchantment.getKey().getNamespace() + ":" + enchantment.getKey().getKey() + ":" + integer));
					});
				...
				extraMeta.addProperty("potion-type", pd.getType().toString());
				extraMeta.addProperty("upgraded", pd.isUpgraded());
				extraMeta.addProperty("extended", pd.isExtended());

				if (pmeta.hasColor()) {
					extraMeta.addProperty("potion-color", pmeta.getColor().asRGB());
				}
				if (pmeta.hasCustomEffects()) {

					JsonArray customEffects = new JsonArray();
					pmeta.getCustomEffects().forEach(potionEffect -> {
						customEffects.add(new JsonPrimitive(potionEffect.getType().getName() + ":" + potionEffect.getAmplifier()
				...
				if (femeta.hasEffect()) {
					...
					if (!effect.getFadeColors().isEmpty()) {
						...
						effect.getFadeColors()
								.forEach(color -> fadeColors.add(new JsonPrimitive(Integer.toHexString(color.asRGB()))));
				...
				/*
				 * 1.11 if(mmeta.hasLocationName()) { extraMeta.addProperty("location-name",
...
//				itemStack.setDurability(data);
				...
				if (itemMetaElement != null && itemMetaElement.isJsonObject()) {
					...
					JsonElement customModelData = metaJson.get("custommodeldata");
					JsonElement damageElement = metaJson.get("damage");
					...
					if (damageElement != null && damageElement.isJsonPrimitive()) {
						((Damageable) meta).setDamage(damageElement.getAsInt());
					}
							...
							if (jsonElement.isJsonPrimitive()) {
								...
								if (enchantString.contains(":")) {
									try {
										...
										String keyNamespace = splitEnchant[0];
										String keyKey = splitEnchant[1];
										
										NamespacedKey key;
										if (keyNamespace.toLowerCase().contains("minecraft")) {
											key = NamespacedKey.minecraft(keyKey);
										} else {
											Plugin keyPlugin = Bukkit.getPluginManager().getPlugin(keyNamespace);
											key = new NamespacedKey(keyPlugin, keyKey);
										}
								...
								if (patternsElement != null && patternsElement.isJsonArray()) {
									...
									jarray.forEach(jsonElement -> {
										...
										if (patternString.contains(":")) {
											...
											Optional<DyeColor> color = Arrays.stream(DyeColor.values())
													.filter(dyeColor -> dyeColor.name().equalsIgnoreCase(splitPattern[0])).findFirst();
...
//													Enchantment enchantment = Enchantment.getByName(splitEnchant[0]);
													...
													if (keyNamespace.toLowerCase().contains("minecraft")) {
														...
														Plugin keyPlugin = Bukkit.getPluginManager().getPlugin(keyNamespace);
														key = keyPlugin == null ? null : new NamespacedKey(keyPlugin, keyKey);
													}
								...
								PotionMeta pmeta = (PotionMeta) meta;
//								if (extraJson.has("potion-type")) {
//									pmeta.setBasePotionData(new PotionData);
//								}
								JsonElement basePotionType = extraJson.get("potion-type");
								JsonElement upgraded = extraJson.get("upgraded");
								JsonElement extended = extraJson.get("extended");
								PotionData basePotionData = new PotionData(
										PotionType.valueOf(basePotionType.getAsString().toUpperCase()), extended.getAsBoolean(),
										upgraded.getAsBoolean());
								pmeta.setBasePotionData(basePotionData);

								if (extraJson.has("potion-color")) {
									JsonElement potionColor = extraJson.get("potion-color");
									pmeta.setColor(Color.fromRGB(potionColor.getAsInt()));
								}
								...
								/*
								 * 1.11 JsonElement locationNameElement = extraJson.get("location-name");
								 * if(locationNameElement != null && locationNameElement.isJsonPrimitive()) {
								 * mmeta.setLocationName(locationNameElement.getAsString()); } JsonElement
								 * colorElement = extraJson.get("color"); if(colorElement != null &&
								 * colorElement.isJsonPrimitive()) {
								 * mmeta.setColor(Color.fromRGB(Integer.parseInt(colorElement.getAsString(),
								 * 16))); }
								 */
							}
						} catch (Exception e) {
