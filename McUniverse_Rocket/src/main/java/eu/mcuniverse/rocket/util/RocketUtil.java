...
public class RocketUtil {
	...
	public static boolean testDistanceToRocket(Player p) {
		...
		try {
			return !p.getLocation().getWorld().getName().equalsIgnoreCase(l.getWorld().getName())
					|| p.getLocation().distance(l) > Core.getConfigManager().getSettings()
							.getDouble("distanceToRocket");
	...
	public static boolean isFloorFlat(Location loc, int tier) {
		return false;
		//		int size = Rocket.configManager.getConfig().getInt("RocketTier" + tier + ".size");
